      ...
      for (var i = 0, len = points.length; i < len; i++) {
        ...
        if (surface) {
          ...
          for (var a = 0; a < count; a++) {
            ...
            var pointOnCircle = {
              x: middleX + realRadius * cos(realAngle),
              y: middleY + realRadius * sin(realAngle)
            };
    ...
    Morpher2.prototype.addPoints = function(points, mustHaveCount) {
      ...
      for (var i = 0, len = points.length; i < len; i++) {
        ...
        for (var p = 1; p < addToSegmentCount; p++) {
          ...
          var extraPoint = {
            x: point0.x + (point1.x - point0.x) * percent2,
            y: point0.y + (point1.y - point0.y) * percent2
          };
...
function createChild(htmlElement, classType) {
  ...
  if (htmlContainer) {
    ...
    tooltipContainer_1.events.on("maxsizechanged", function() {
      getValue(sprite_1.tooltip).setBounds({
        x: 0,
        y: 0,
        width: tooltipContainer_1.maxWidth,
        height: tooltipContainer_1.maxHeight
    ...
    if (options.onlyShowOnViewport) {
      if (!isElementInViewport(htmlContainer, options.viewportTarget)) {
        ...
        var disposers = [addEventListener(window, "DOMContentLoaded", function() {
